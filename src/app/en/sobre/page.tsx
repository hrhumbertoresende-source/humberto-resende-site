...
export const metadata: Metadata = {
  ...
  description:
    "Architect and scenographer leading Estúdio Dorama, Humberto Resende has signed projects for brands like Coca-Cola, Google, Netflix, Adidas, and Chanel.",
};

export default async function SobrePageEn() {
  const [content, site] = await Promise.all([getSobreContent("en"), getSiteContent("en")]);
        ...
        <div className="mt-24">
          ...
          <div className="mt-10 grid grid-cols-1 gap-x-12 gap-y-8 md:grid-cols-[1fr_1.5fr]">
            <div className="space-y-4 text-sm text-neutral-600">
              {contact.address && (
                <div>
                  <p className="text-xs font-bold uppercase tracking-wide text-neutral-900">Address</p>
                  <p className="mt-1">{contact.address}</p>
                </div>
              )}
              {contact.email && (
                <div>
                  <p className="text-xs font-bold uppercase tracking-wide text-neutral-900">Email</p>
                  <p className="mt-1">{contact.email}</p>
                </div>
              )}
              {contact.phone && (
                <div>
                  <p className="text-xs font-bold uppercase tracking-wide text-neutral-900">Phone</p>
